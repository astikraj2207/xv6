...
void scheduler(void) {
        ...
        for (;;) {
            ...
            acquire(&ptable.lock);
                ...
                struct proc *temp = 0;//temporary process variable to store the required process which will be scheduled now
                for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
                    if(p->state!=RUNNABLE){continue;}
        ...
        c->proc = 0;
        for (;;) {
            int flag=1;//a flag variable so that the scheduler always checks for priority 3 and then for 2 and then 1
            // Enable interrupts on this processor.
            sti();
            ...
            acquire(&ptable.lock);
                for (int i=i3;i<NPROC+i3  && flag==1; i++) {
                    struct proc *temp = &ptable.proc[i%NPROC];//modulo is done to check cyclically
...
static void wakeup1(void *chan) {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
        if (p->state == SLEEPING && p->chan == chan) {
            p->state = RUNNABLE;
            #ifdef DML//check before submitting the report, a little bit unsure
                p->priority = 3;
            #endif
        }
}
...
int wait2(int *retime, int *rutime, int *stime, int *ctime) {
    struct proc *p;
    int havekids, pid;
    struct proc *curproc = myproc();
